 ...
 */
define(['angular', 'text!module/facilitator/servicesanswer.html'], function (angular, tpl) {
    ...
    return {
        controller: ['$scope', '$routeParams', '$http', '$modal', 'alertSrv', function ($scope, $routeParams, $http, $modal, alertSrv) {		
			...
			$scope.bind = function(){
			    ...
			    $http.get('../webapi/facilitator/services.ashx?act=all&getFlag=11&getMode=1')
				...
				var modalInstance = $modal.open({
					...
					controller:['$modalInstance',function($modalInstance){
						...
						vm.refuseService = function (id) {
						    $http.post('../webapi/facilitator/services.ashx?act=refuseService&servicesId=' + id + "&getMode=1")
						...
						vm.acceptService = function (id) {
						    vm.button = true;
						    $http.post('../webapi/facilitator/services.ashx?act=acceptService&servicesId=' + id + "&getMode=1")
							.success(function (resp) {
							    if (resp.result != undefined && resp.result == "9999") {
							        window.location.href = "/userlogin.aspx";
							        return false;
